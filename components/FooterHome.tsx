...
export function FooterHome() {
  return (
    <footer className="relative z-10 w-full bg-grey-900 px-2 pb-2 pt-2 text-white">
      <div className="rounded-[2rem] bg-black px-6 py-16 | md:px-12 md:py-24 | xl:rounded-[3rem]">
          ...
          <div className="flex flex-col gap-10 | lg:col-span-5">
            ...
            <form className="relative w-full max-w-sm" onSubmit={(e) => e.preventDefault()}>
              <input
                ...
                className="h-16 w-full rounded-full bg-grey-900/80 pl-8 pr-20 text-lg text-white outline-none ring-1 ring-white/10 transition-shadow focus:ring-mint/50"
              />
              <button
                type="submit"
            ...
            <div className="flex flex-wrap gap-2.5">
              {social.map(({ href, icon: Icon, label }) => (
                <Link
                  key={label}
                  href={href}
                  className="flex items-center gap-1.5 rounded-full bg-white px-3 py-1.5 text-grey-900 transition-transform hover:scale-110"
                >
        ...
        {/* Giant Logo */}
        <div className="mt-24 w-full px-0 | xl:mt-40">
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-6 border-t border-white/5 pt-8 text-[11px] uppercase tracking-wider text-white/40 | md:flex-row">
          <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 | md:justify-start">
            <p>© {new Date().getFullYear()} Rise at Seven Ltd. All rights reserved</p>
          ...
          <p className="font-medium whitespace-nowrap">Website MadeByShape</p>
        </div>
      </div>
    </footer>
  );
